...
def next_pos(current_pos, move):
    next_col = wrap(current_pos[0] + move[0], columns[current_pos[1]])
    next_row = wrap(current_pos[1] + move[1], rows[current_pos[0]])
    return (next_col, next_row)


def do_move():
    global current_move, current_pos, moves, mp
    maybe_next = next_pos(current_pos, moves[current_move])
